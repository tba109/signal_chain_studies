...
if __name__ == '__main__':
    ...
    parser.add_argument("--path",type=str,help="path to directories for files",default="./C2--waveforms--00000.txt")
    parser.add_argument("--nhdr",type=int,help='number of header lines to skip in the raw file',default=5)
    args = parser.parse_args()

    flist = os.listdir(args.path)
    for fli in flist: 
        pfli = args.path + '/' + fli
        x,y,header = read_waveform(pfli,args.nhdr)
        print header
        plt.plot(x,y)
        plt.show()
